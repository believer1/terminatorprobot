...
@user_admin
@loggable
def mod(bot: Bot, update: Update, args: List[str]) -> str:
    message = update.effective_message  # type: Optional[Message]
    user = update.effective_user  # type: Optional[User]
    chat = update.effective_chat  # type: Optional[Chat]
    conn = connected(bot, update, chat, user.id)
    ...
    message.reply_text(tld(chat.id, f"👷🏻‍♂️ Moderator. in *{chatD.title}*!"), parse_mode=ParseMode.MARKDOWN)
    return f"<b>{html.escape(chatD.title)}:</b>" \
            "\n#mod" \
           f"\n<b>Admin:</b> {mention_html(user.id, user.first_name)}" \
           f"\n<b>User:</b> {mention_html(user_member.user.id, user_member.user.first_name)}"
...
*Admin only:*
 ...
 - /mod: Moderators user replied to
...
MOD_HANDLER = CommandHandler("mod", mod, pass_args=True, filters=Filters.group)
