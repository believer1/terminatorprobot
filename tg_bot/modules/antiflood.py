...
def check_flood(bot: Bot, update: Update) -> str:
    ...
    try:
        ...
        if getmode == 1:
            ...
            execstrings = tld(update.effective_message, "Banned till {}" ).format(getvalue)
            tag = "TBAN"
        elif getmode == 5:
            mutetime = extract_time(msg, getvalue)
            bot.restrict_chat_member(chat.id, user.id, until_date=mutetime, can_send_messages=False)
            execstrings = tld(update.effective_message, "Muted till {}" ).format(getvalue)
            tag = "TMUTE"
        msg.reply_text(tld(update.effective_message, "Don't try to flood in font of me *{}*").format(execstrings))
